(() => {
  ...
  document.querySelectorAll('[data-target-option]').forEach(button => {
    button.addEventListener('click', () => {
      ...
      document.querySelectorAll('[data-target-value]').forEach(el => { el.textContent = m ? m[1] : value; });
      document.querySelectorAll('[data-target-unit]').forEach(el => { el.textContent = m ? m[2].toUpperCase() : ''; });
  ...
  if (hero && !document.querySelector('[data-auto-visual]')) {
    ...
    visual.innerHTML = `
      ...
      <div class="visual-body">
        ...
        <div class="target-panel"><div class="target-kicker">A target you can understand</div><div class="target-size"><span data-target-value>${initial.replace(/KB|MB/i,'')}</span> <small data-target-unit>${initial.match(/KB|MB/i)?.[0] || 'KB'}</small></div><p class="target-copy" data-target-note="${isPdf ? 'pdf' : 'image'}">${isPdf ? `A ${initial} target is useful when a portal, application, or email system has a hard PDF limit.` : `A ${initial} target is useful when a form, profile, application, or website sets a hard image limit.`}</p><div class="target-options">${options.map(v => `<button class="target-option" data-target-option="${v}"${v===initial?' aria-current="true"':''}>${v}</button>`).join('')}</div></div>
      </div>`;
